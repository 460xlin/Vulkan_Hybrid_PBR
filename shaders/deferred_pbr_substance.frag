...
void main() {
	...
	vec3 local_light_pos = vec3(10.0f);
	vec3 v_pos = texture(samplerPosition, inUV).xyz;
	...
	vec3 V = -normalize(v_pos - ubo.eyePos);
    ...
    vec3 base = texture(samplerAlbedo, inUV).xyz;
	...
	vec3 envdiff = texture(samplerCubemap, N).xyz;
    ...
    // ).xyz;
	vec3 envspec = texture(samplerCubemap, refl).xyz;
...
#define PHONG
    ...
    // IBL lighting
    vec2 brdf = texture(samplerBrdfLUT, vec2(roughness, 1.0 - NdV)).xy;
